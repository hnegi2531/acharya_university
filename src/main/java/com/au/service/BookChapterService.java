...
import com.au.repository.BookChapterRepository;
...
public class BookChapterService {
	...
	private Logger log = LoggerFactory.getLogger(BookChapterService.class);

	public static final String value = "BookChapter";
	...
	@Autowired
	private BookChapterRepository book_repo;

	@Autowired
	private IncentiveApproverRepository ApproverRepository;

	public List<BookChapter> savePublication(@Valid List<BookChapter> ademail) {
		ademail.stream().forEach(bookChapter -> {
			if (book_repo.getCountOfIsbnNumber(bookChapter.getIsbn_number(),bookChapter.getEmp_id()) >= 1) {
				throw new RuntimeException("IsbnNumber already created " + bookChapter.getIsbn_number() + "!!!");
			}

		});
		return book_repo.saveAll(ademail);
	}

	public List<BookChapter> getAllActiveBookChapter() {
		return book_repo.getAllActiveBookChapter();
	}

	public BookChapter get(Integer id) {
		return book_repo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Book Chapter Not Found:" + id));
	}

	public BookChapter updateBookChapter(BookChapter book) {
		return book_repo.save(book);
	}

	public void deactivate(Integer id) {
		BookChapter ademail = book_repo.findById(id)
				.orElseThrow(() -> new ResourceNotFoundException("Book Chapter Not Found:" + id));
		book_repo.deactivate(id);
	}

	public void activate(Integer id) {
		BookChapter ademail = book_repo.findById(id)
				.orElseThrow(() -> new ResourceNotFoundException("Book Chapter Not Found:" + id));
		book_repo.activate(id);
	}

//	public ResponseEntity<Object> getAllDataFilteredByKeyword(Pageable pageable, Object keyword, Integer percentageFilter) {
//		Page<Map<String, Object>> oc_filtered_response = book_repo.getAllDataFilteredByKeyword(pageable, keyword,percentageFilter);
...
//		Page<Map<String, Object>> oc_sorted_response = book_repo.getAllSortedData(pageable,percentageFilter);
//		return ResponseHandler.generateResponseForIndex1(true, HttpStatus.OK, oc_sorted_response);
//	}

	public void uploadFile(MultipartFile multipartFile, Integer book_chapter_id) {

		BookChapter entity = book_repo.findById(book_chapter_id)
				.orElseThrow(() -> new ResourceNotFoundException("Book chapter not found"));
		try {
			...
			entity.setAttachment_path(LocalDate.now() + "/" + book_chapter_id + "/" + fileName);
			uploadFileTos3bucket(fileName, file, book_chapter_id);
	...
	private void uploadFileTos3bucket(String fileName, File file, Integer book_chapter_id) {
		final String uniqueFileName = value + "/" + LocalDate.now() + "/" + book_chapter_id + "/" + fileName; // file.getName()
	...
	public List<Map<String, Object>> bookChapterDetailsBasedOnEmpId(List<Integer> emp_id, Integer percentageFilter) {
		List<Map<String, Object>> bookChapterDetails = book_repo.bookChapterDetailsBasedOnEmpId(emp_id,percentageFilter);

		List<Map<String, Object>> modifiedPublicationDetails = new ArrayList<>();

		for (Map<String, Object> detail : bookChapterDetails) {
	...
	public List<Map<String, Object>> fetchAllBookChapter(Integer percentageFilter) {
		List<Map<String, Object>> bookChapterDetails = book_repo.fetchAllBookChapter(percentageFilter);
	...
	public List<Map<String, Object>> bookChapterBasedOnEmpId(Integer emp_id) {
		return book_repo.bookChapterBasedOnEmpId(emp_id);
	}
