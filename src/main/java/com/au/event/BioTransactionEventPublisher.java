...
@Component
@Slf4j
public class BioTransactionEventPublisher {
    ...
    @Async
    @EventListener
    public void handleBioTransactionEventEventPublisher(BioTransactionEvent bioTransactionEvent) throws ParseException {

        Integer empId = bioTransactionEvent.getEmpId();
        String bioDate = bioTransactionEvent.getDate();
        String empCode = null;

        LocalDate today = LocalDate.parse(bioDate);
        LocalDate targetDate = LocalDate.of(2025, 5, 2);
        boolean isBeforeTarget = today.isBefore(targetDate);

        if (empId != null) {
            empCode = isBeforeTarget ? employeeDetailsRepository.findContractEmpcodeByEmpId(empId) : employeeDetailsRepository.findEmpcodeByEmpId(empId);
        }
        ...
        ResponseEntity<ApiResponsephp> responseEntity = restTemplate.exchange(API_URL + bioDate + "&ach=2&code=" + empCode, HttpMethod.GET, entity, ApiResponsephp.class);
        ...
        if (apiResponse != null && apiResponse.isSuccess()) {
            ...
            apiResponse.getData().forEach(b -> {
                ...
                String trnDate = dateFormat.format(b.getTrn_date());
                String empcode = b.getEmpcode();
                BiometricTransaction existingTransaction = biometricTransactionRepository.findFirstByEmpcodeAndTrnDateAndTrnTime(empcode, trnDate, adjustedTime);
            ...
            Date date = dateFormat.parse(bioTransactionEvent.getDate());
            ...
            if (empCode != null) {
                biometricTransaction = biometricTransactionRepository.getByTrnDateAndEmpcode(bioDate, empCode);
            } else biometricTransaction = biometricTransactionRepository.getByTrnDate(bioDate);

            System.out.println(bioDate);

            setBioTransDataOnBasisOfTrnDate(biometricTransaction, bioTransactionEvent.getDate(), month, year, calendar, bioTransactionEvent, isBeforeTarget);
    ...
    private void setBioTransDataOnBasisOfTrnDate(List<BiometricTransaction> biometricTransactions, String formattedDate, Integer month, Integer year, Calendar calendar, BioTransactionEvent bioMetricAttendanceEvent, boolean isBeforeTarget) {
        try {
            if (ObjectUtils.isNotEmpty(biometricTransactions)) {
                ...
                Map<String, List<BiometricTransaction>> bioTransData = biometricTransactions.stream()
                        .sorted(Comparator.comparing(BiometricTransaction::getTrnTime))
                        .collect(Collectors.groupingBy(BiometricTransaction::getEmpcode, LinkedHashMap::new, Collectors.toList()));
                bioTransData.forEach((k, v) -> {
                    try {
                        if (ObjectUtils.isNotEmpty(v) && !checkReleivingDate(k, formattedDate)) {
                            ...
                            Time firstEntryTrnTime = ObjectUtils.isNotEmpty(firstEntry.getTrnTime()) ? firstEntry.getTrnTime() : null;
                            ...
                            Time lastEntryTrnTime = ObjectUtils.isNotEmpty(lastEntry) && ObjectUtils.isNotEmpty(lastEntry.getTrnTime())
                                    ? lastEntry.getTrnTime() : null;
                            ...
                            if (ObjectUtils.isNotEmpty(shiftId)) {
                                BiometricAttendance biometricAttendance = biometricAttendanceRepository.findFirstByDateAndEmpCode(formattedDate, empcode);
                                ...
                                biometricAttendance.setShiftStartTime(ObjectUtils.isNotEmpty(shift.getGrace_time()) ? shift.getGrace_time() : null);
                                biometricAttendance.setShiftEndTime(ObjectUtils.isNotEmpty(shift.getShiftEndTime()) ? shift.getShiftEndTime() : null);
                                biometricAttendance.setGrace_start_time(ObjectUtils.isNotEmpty(shift.getShiftStartTime()) ? shift.getShiftStartTime() : null);
                                biometricAttendance.setShiftId(shiftId);
                                biometricAttendance.setStartTime(ObjectUtils.isNotEmpty(modifiedEntryTime) ? formatter.format(modifiedEntryTime) : null);
                                biometricAttendance.setEndTime(ObjectUtils.isNotEmpty(modifiedExitTime) ? formatter.format(modifiedExitTime) : null);

                                biometricAttendance.setEmpId(empId);
                                biometricAttendance.setMonth(month);
                                biometricAttendance.setYear(year);
                                ...
                                LocalTime targetTime = LocalTime.of(19, 55);

                                if (nowInIST.toLocalTime().isAfter(targetTime)) {
                                    /* Flex Shift */
                                    if (!punchCardStatus.isEmpty() && punchCardStatus.equalsIgnoreCase("flexible")) {
                                        ...
                                        if ((StringUtils.isNotEmpty(leave_type_short) || day.equalsIgnoreCase("Sunday")) &&
                                                (durationTotalMinutes >= targetTotalMinutesForCompOff) && (leave_type_short != null &&
                                                leave_type_short.equalsIgnoreCase("GH") || leave_type_short != null &&
                                                leave_type_short.equalsIgnoreCase("DH") || day.equalsIgnoreCase("Sunday"))) {
                                            biometricAttendance.setCompoffStatus(true);
                                        }
                                    }

                                    if (StringUtils.isNotEmpty(punchCardStatus) && StringUtils.equals(punchCardStatus, "Optional")) {
                                        if ((ObjectUtils.isNotEmpty(modifiedEntryTime) && !modifiedEntryTime.equals(modifiedExitTime)) && ObjectUtils.isNotEmpty(modifiedExitTime)) {
                                            biometricAttendance.setAttendanceStatus(1d);
                                            biometricAttendance.setPresentStatus("P");
                                        } else {
                                            biometricAttendance.setAttendanceStatus(0d);
                                    ...
                                    if (StringUtils.isNotEmpty(punchCardStatus) && StringUtils.equals(punchCardStatus, "No Swipe")) {
                                        biometricAttendance.setAttendanceStatus(1d);
                                        biometricAttendance.setPresentStatus("P");
                                        Date d = dateFormat.parse(biometricAttendance.getStartTime());
                                        Date d1 = dateFormat.parse(biometricAttendance.getEndTime());
                                    ...
                                    if (StringUtils.isEmpty(punchCardStatus) || StringUtils.equalsIgnoreCase(punchCardStatus, "Mandatory")) {
                                        System.out.println(k + " modifiedExitTime " + modifiedExitTime + " endTime " + endTime + " modifiedEntryTime " + modifiedEntryTime + " startTime " + startTime + " formattedDate " + formattedDate);
                                        if (ObjectUtils.isNotEmpty(biometricAttendance.getShiftId()) && biometricAttendance.getShiftId() != shiftId) {
                                            Shift shiftChange = shiftRepository.getShiftData(biometricAttendance.getShiftId());
                                            biometricAttendance.setShiftStartTime(ObjectUtils.isNotEmpty(shiftChange.getGrace_time()) ? shiftChange.getShiftStartTime() : null);
                                            biometricAttendance.setShiftEndTime(ObjectUtils.isNotEmpty(shiftChange.getShiftEndTime()) ? shiftChange.getShiftEndTime() : null);
                                        ...
                                        if ((modifiedExitTime.equals(endTime.toLocalTime().withSecond(0)) || modifiedExitTime.isAfter(endTime.toLocalTime().withSecond(0))) && (modifiedEntryTime.isBefore(startTime.toLocalTime().withSecond(0)) || modifiedEntryTime.equals(startTime.toLocalTime().withSecond(0)))) {
                                            biometricAttendance.setAttendanceStatus(1d);
                                            biometricAttendance.setPresentStatus("P");
                                        } else if ((modifiedExitTime.isAfter(firstHalf) || modifiedExitTime.equals(firstHalf)) && ((modifiedExitTime.isBefore(endTime.toLocalTime().withSecond(0)) && modifiedEntryTime.isAfter(modifiedExitTime) && !modifiedEntryTime.equals(modifiedExitTime)) || ((modifiedEntryTime.isBefore(startTime.toLocalTime().withSecond(0)) || (modifiedEntryTime.equals(startTime.toLocalTime().withSecond(0)))) && modifiedExitTime.isBefore(endTime.toLocalTime().withSecond(0)) && !modifiedEntryTime.equals(modifiedExitTime) && !modifiedExitTime.isBefore(firstHalf)))) {
                                            biometricAttendance.setAttendanceStatus(0.5);
                                            biometricAttendance.setPresentStatus("p/a");
                                        } else if ((modifiedExitTime.isAfter(endTime.toLocalTime().withSecond(0)) || modifiedExitTime.equals(endTime.toLocalTime().withSecond(0))) && (modifiedEntryTime.isBefore(firstHalf) || modifiedEntryTime.equals(firstHalf))) {
                                            biometricAttendance.setAttendanceStatus(0.5);
                                            biometricAttendance.setPresentStatus("a/p");
                                        } else {
                                            biometricAttendance.setAttendanceStatus(0d);
                                        ...
                                        String leave_type_short = holidayCalenderRepository.getleaveType(formattedDate);
                                        String day = calendar.getDisplayName(Calendar.DAY_OF_WEEK, Calendar.LONG, Locale.getDefault());
                                        if ((StringUtils.isNotEmpty(leave_type_short) || day.equalsIgnoreCase("Sunday")) && (durationTotalMinutes >= targetTotalMinutes) && (leave_type_short != null && leave_type_short.equalsIgnoreCase("GH") || leave_type_short != null && leave_type_short.equalsIgnoreCase("DH") || day.equalsIgnoreCase("Sunday"))) {
